...
import { NODE_TYPES, NODE_TYPE_LABELS, GENERATION_YEARS } from '@contracts/dcgen';
import type { NodeType } from '@contracts/dcgen';

// ---------------- zod schema（對齊後端 itConfigInput） ----------------
const nodeSchema = z.object({
  nodeType: z.enum(NODE_TYPES),
  rackCount: z.coerce.number({ error: '請輸入機架數' }).int('機架數需為整數').nonnegative('機架數不可為負'),
  rackTdp: z.coerce.number({ error: '請輸入 RackTDP' }).nonnegative('RackTDP 不可為負'),
});

const itConfigSchema = z.object({
  name: z.string().min(1, '請輸入配置名稱').max(191, '名稱過長'),
  datacenterType: z.enum(DATACENTER_TYPES),
  model: z.enum(['Canonical', 'Reference']),
  generation: z.string().min(1, '請選擇年份'),
  rackSize: z.coerce.number({ error: '請輸入 RackSize' }).int('RackSize 需為整數').positive('RackSize 需大於 0'),
  rackType: z.enum(['Cloud', 'HPC']),
  floorSpace: z.coerce.number({ error: '請輸入 floorSpace' }).positive('floorSpace 需大於 0'),
  sourceUrl: z
    .string()
    .max(512, 'URL 過長')
    .optional()
  ...
  nodeTypes: z.array(nodeSchema).min(1, '至少需要一個節點類型'),
});
...
export default function ItConfigDrawer({ open, row, onClose, onSaved }: ItConfigDrawerProps) {
  ...
  return (
    <FormDrawer
      open={open}
      onClose={onClose}
      title={row ? `編輯 IT 配置：${row.name}` : '新增 IT 配置'}
      subtitle="Canonical 為論文基準配置；Reference 為真實系統。標 * 為必填。"
      footer={
        <div className="flex items-center justify-between gap-3">
          <p className="font-mono text-xs text-text-1">
            總機架 <span className="text-text-0">{fmtNum(totals.racks)}</span> · 總 IT 功率{' '}
    ...
    >
      <form id="itconfig-form" onSubmit={onSubmit} className="flex flex-col gap-5" noValidate>
        {/* 名稱 */}
        <Field label="配置名稱 *" error={errors.name?.message}>
          <input
            className={inputClass(!!errors.name)}
            placeholder="例如：Canonical AI Training 2027"
            {...register('name')}
          />
        </Field>

        {/* DC 類型 */}
        <Field label="DC 類型 *" error={errors.datacenterType?.message}>
          <Select
          ...
          >
            <SelectTrigger className={inputClass(!!errors.datacenterType)}>
              <SelectValue placeholder="選擇 DC 類型" />
            </SelectTrigger>
            <SelectContent>
              {DATACENTER_TYPES.map((t) => (
                <SelectItem key={t} value={t}>
                  {DATACENTER_TYPE_LABELS[t]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        {/* 來源 radio */}
        <Field label="來源 *" error={errors.model?.message}>
          <div className="grid grid-cols-2 gap-2">
            {(['Canonical', 'Reference'] as const).map((m) => (
              ...
              >
                {m === 'Canonical' ? 'Canonical（論文基準）' : 'Reference（真實系統）'}
              </button>
            ))}
          </div>
        </Field>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="年份 *" error={errors.generation?.message}>
          ...
          <Field label="RackSize（U）*" error={errors.rackSize?.message}>
            <input
              type="number"
              step={1}
              min={1}
        ...
        {/* RackType / floorSpace */}
        <div className="grid grid-cols-2 gap-3">
          <Field label="RackType *" error={errors.rackType?.message}>
            <Select
              value={rackType}
              onValueChange={(v) => setValue('rackType', v as 'Cloud' | 'HPC', { shouldValidate: true })}
            >
              <SelectTrigger className={inputClass(!!errors.rackType)}>
                <SelectValue placeholder="機架類型" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="Cloud">Cloud（{RACK_TYPE_LABELS.Cloud}）</SelectItem>
                <SelectItem value="HPC">HPC（{RACK_TYPE_LABELS.HPC}）</SelectItem>
              </SelectContent>
            </Select>
          </Field>
          <Field label="floorSpace（m²/rack）*" error={errors.floorSpace?.message}>
            <input
              type="number"
              step="any"
              min={0}
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            <span className="text-xs font-medium tracking-wide text-text-1">Node types *</span>
            <Button
              type="button"
              variant="outline"
              size="sm"
          ...
          {errors.nodeTypes?.root?.message && (
            <p className="mb-2 text-xs text-red">{errors.nodeTypes.root.message}</p>
          )}
          {typeof errors.nodeTypes?.message === 'string' && (
            <p className="mb-2 text-xs text-red">{errors.nodeTypes.message}</p>
          )}
                  ...
                  <div className="grid grid-cols-[1fr_88px_96px_36px] items-start gap-2 rounded-lg border border-line bg-bg-2 p-2.5">
                    ...
                    {(errors.nodeTypes?.[i]?.rackCount || errors.nodeTypes?.[i]?.rackTdp) && (
                      <p className="col-span-4 text-xs text-red">
                        {errors.nodeTypes?.[i]?.rackCount?.message ?? errors.nodeTypes?.[i]?.rackTdp?.message}
                      </p>
                    )}
                  </div>
                </motion.div>
